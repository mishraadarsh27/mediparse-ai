...
# ============================================
# WORKFLOW STAGES & STATUS MAPPINGS
# ============================================
...
def _admission_number() -> str:
    """Generate a random admission number."""
    chars = string.ascii_uppercase + string.digits
    return "ADM-" + "".join(random.choices(chars, k=8))
...
def admit_patient(case: dict, diagnosis: str = "", ward: str = "General") -> dict:
...
def request_enhancement(case: dict, provisional_bill: float, reason: str = "", documents: list = None) -> dict:
    """Hospital raises an enhancement request if treatment cost increases."""
...
def get_case_summary(case: dict) -> dict:
    ...
    # Calculate revenue loss based on case status
    if case["status"] in ["settled", "closed"]:
        ...
        revenue_loss = max(0, final_bill - (tpa_payable + copay + deductions))
    else:
        revenue_loss = 0
