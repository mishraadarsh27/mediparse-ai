...
const DOCUMENT_TYPES = ['Lab Report','Hospital Bill','Prescription','Discharge Summary']
...
export default function UploadPage({ onSuccess, toast }) {
  ...
  function addFiles(incoming) {
    ...
    if (!validFiles.length) { 
      toast('Only PDF, PNG, and JPG files are supported', 'error')
      return 
    }
    if (mode === 'single') {
      setFiles([validFiles[0]])
    } else {
      setFiles(prev => [...prev, ...validFiles].slice(0, MAX_BATCH_FILES))
    }
  }
  ...
  return (
    <div className="anim-fade" style={{ maxWidth: MAX_WIDTH, margin:'0 auto', padding:'10px 0 40px' }}>
      <style>{`
        @keyframes floating {
      ...
      >
        ...
        {files.length === 0 ? (
          ...
          <div>
            ...
            {mode === 'batch' && files.length < MAX_BATCH_FILES && (
