...
const BACKGROUND_IMAGES = {
  hospital: "https://images.unsplash.com/photo-1516549655169-df83a0774514?auto=format&fit=crop&q=80&w=2000",
  tpa: "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&q=80&w=2000"
}

const LOGIN_TIMEOUT_MS = 1500
const SYSTEM_LOG_ID = '8828-SYNC'
...
export default function LoginPage({ onLogin, defaultRole, onBack }) {
  ...
  const bgImg = isHosp ? BACKGROUND_IMAGES.hospital : BACKGROUND_IMAGES.tpa
  ...
  const handleLogin = (e) => {
    ...
    if (isHosp && hospitalCode.trim().length < 3) {
      alert("Please enter a valid Hospital Code.")
      return
    }
    if (!isHosp) {
      if (tpaCode.trim().length < 3) {
        alert("Please enter a valid TPA Client Code.")
        return
      }
      if (auditorCode.trim().length < 3) {
        alert("Please enter a valid Auditor Licence Key.")
        return
      }
    }
    ...
    setLoading(true)
    // TODO: Replace this setTimeout with a real API call to your backend/Supabase Auth
    setTimeout(() => { 
      onLogin(role)
      setLoading(false) 
    }, LOGIN_TIMEOUT_MS)
      ...
      <div style={{ width: '100%', maxWidth: 440, padding: 24, zIndex: 10, animation: 'fadeUp 0.8s ease' }}>
        ...
        >
          ...
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            ...
            <p style={{ 
              color: 'rgba(255,255,255,0.7)', fontSize: 10, fontWeight: 950, marginTop: 10, 
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
             ...
             <div className="anim-fade" style={{animationDelay: '0.2s'}}>
               ...
               <input 
                 required 
                 value={isHosp ? hospitalCode : auditorCode}
                 onChange={(e) => isHosp ? setHospitalCode(e.target.value) : setAuditorCode(e.target.value)}
                 placeholder={isHosp ? "HSP-1080-..." : "ID-453-ZZZ..."} 
                 style={inputStyle(accent)} 
               />
             </div>
             <div className="anim-fade" style={{animationDelay: '0.3s'}}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  {!isHosp && <button type="button" style={{ background: 'none', border: 'none', color: accent, fontSize: 10, fontWeight: 950, cursor: 'pointer', textShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>RESET TOKEN?</button>}
                </div>
               <input 
                 required 
                 type="password" 
        ...
        <div style={{ marginTop: 44, display: 'flex', gap: 32, justifyContent: 'center' }}>
          <button onClick={() => setModal({ title: 'Enterprise Technical Support', content: `MediParse AI support is active. For direct technician assistance, contact support@mediparse.ai. System log ID: ${SYSTEM_LOG_ID}.` })} style={footerBtnStyle}>Emergency Support</button>
